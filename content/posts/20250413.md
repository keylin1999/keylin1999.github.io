...
記得剛畢業的時候面試 nvidia 面試官問了很多關於 C++ 的東西，然後希望現場寫一個矩陣的 class，那時候我答得不太好，面試官在我寫的過程中幫我做一些修改最後才動起來。雖然只是一些語法上的問題，但其實就顯現出我其實對 C++ 並沒有真的到很熟。後來我現場寫題目的環節有答對，但很多幾題沒有想到最佳解所以最後收了拒絕信。現在我開始工作一陣子以後應該回頭把這些語法複習，然後在準備的過程中也刷題準備接下來其他間公司的面試。

預計會先快速複習 C++ 的 class 語法、然後接下來會看 modern C++ 的內容，最後預計會看 linking 的相關內容。

這篇文章會寫 class 的 syntax, inheritance 和 polymorphism，還會寫紀錄 Copy Constructor and Copy Assignment 的寫法
...
原話：NOTE: It is very important that a default virtual destructor was included after adding the virtual `Area()` method to the base class. Whenever a base class includes even a single  virtual method, it must include a virtual destructor so that the correct destructor(s) are called in the correct order when the object is  eventually deleted.

小結：有 virtual method 一定要有一個 virtual destructor

為什麼一定要用 virtual，假設以下 example，因為我們假設 a 的 type 是 Animal，所以最後他 call 的 function 還是 Animal 的實作:
